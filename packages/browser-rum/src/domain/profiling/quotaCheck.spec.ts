import { mockClock, interceptRequests, NETWORK_ERROR_FETCH_MOCK } from '@openobserve/browser-core/test'
...
describe('checkProfilingQuota', () => {
  ...
  it('routes through proxy when proxy is configured as a string, adding o2forwardSubdomain=quota', async () => {
    interceptor.withFetch(backendResponse(true, 'quota_ok'))
    await checkProfilingQuota(mockRumConfiguration({ proxy: 'http://proxy.example.com' }), 'session-abc')
    expect(interceptor.requests[0].url).toBe(
      'http://proxy.example.com/?o2forward=%2Fapi%2Fv2%2Fprofiling%2Fquota%3Fsession_id%3Dsession-abc&o2forwardSubdomain=quota'
    )
  })
  ...
  it('normalizes string proxy URLs', async () => {
    ...
    expect(interceptor.requests[0].url).toBe(
      `${location.origin}/proxy?o2forward=%2Fapi%2Fv2%2Fprofiling%2Fquota%3Fsession_id%3Dsession-abc&o2forwardSubdomain=quota`
    )
  })

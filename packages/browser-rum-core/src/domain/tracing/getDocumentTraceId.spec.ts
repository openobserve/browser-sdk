...
import { getTimeOrigin } from '@openobserve/js-core/time'
import {
...
describe('getDocumentTraceId', () => {
  it('gets a trace id found from a document comment', () => {
    expect(
      getDocumentTraceId(
        createDocument(
          `<!-- OPENOBSERVE;trace-id=foo;trace-time=${getTimeOrigin()} -->
          ${HTML_DOCTYPE}
          <html>
            <head>
            </head>
    ...
    expect(
      getDocumentTraceId(
        createDocument(
          ...
          <html>
            <head>
              <meta name="o2-trace-id" content="foo" />
              <meta name="o2-trace-time" content="${getTimeOrigin()}" />
            </head>
            <body>
            </body>
          </html>`
  ...
  it('uses the meta strategy in priority', () => {
    expect(
      getDocumentTraceId(
        createDocument(
          `<!-- OPENOBSERVE;trace-id=comment;trace-time=${getTimeOrigin()} -->
          ${HTML_DOCTYPE}
          <html>
            <head>
              <meta name="o2-trace-id" content="meta" />
              <meta name="o2-trace-time" content="${getTimeOrigin()}" />
            </head>
            <body>
            </body>
  ...
  it('ignores the trace id if it has been created too long ago', () => {
    expect(
      getDocumentTraceId(
        createDocument(`<!-- OPENOBSERVE;trace-id=foo;trace-time=${
    ...
    expect(
      getDocumentTraceDataFromMeta(
        createDocument(
          ...
          <html>
            <head>
              <meta name="o2-trace-id" content="123" />
              <meta name="o2-trace-time" content="456" />
            </head>
            <body>
            </body>
          </html>`

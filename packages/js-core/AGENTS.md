# `@openobserve/js-core` package

`@openobserve/js-core` is a **stable, runtime-agnostic** utility package consumed by various Datadog
JavaScript SDKs such as `electron-sdk`, `openfeature-js-client` as well as the Browser SDK packages.
It is versioned **independently** from the `@openobserve/browser-*` release cadence.
...
All APIs live under a named sub-path (e.g. `@openobserve/js-core/time`). There is no root entry
...
4. Add `"@openobserve/js-core/<name>"` to the `paths` map in the root `tsconfig.base.json`, pointing at
...
yarn workspace @openobserve/js-core build

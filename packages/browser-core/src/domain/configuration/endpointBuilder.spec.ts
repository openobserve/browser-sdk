...
describe('endpointBuilder', () => {
  ...
  describe('query parameters', () => {
    ...
    it('should add the provided encoding', () => {
      ...
      ).toContain('&o2-evp-encoding=deflate')
    })
  ...
  describe('proxy configuration', () => {
    it('should replace the intake endpoint by the proxy and set the intake path and parameters in the attribute o2forward', () => {
      expect(
        ...
        `https://proxy.io/path\\?o2forward=${encodeURIComponent(
          `/api/v2/rum?o2source=(.*)&o2-api-key=${clientToken}` +
    ...
    it('normalizes the proxy url', () => {
      ...
      expect(endpoint.startsWith(`${location.origin}/path?o2forward`)).toBeTrue()
    })
    ...
    it('should allow to fully control the proxy url', () => {
      ...
      expect(
        ...
        `https://proxy.io/prefix/api/v2/rum/suffix\\?o2source=(.*)&o2-api-key=${clientToken}&o2-evp-origin-version=(.*)&o2-evp-origin=browser&o2-request-id=(.*)&batch_time=(.*)`
      )
    })
  })
  ...
  describe('source configuration', () => {
    it('should use the default source when no configuration is provided', () => {
      ...
      expect(endpoint).toContain('o2source=browser')
    })
  ...
  describe('createReplicaEndpointBuilder', () => {
    ...
    it('uses the replica client token', () => {
      ...
      expect(createReplicaEndpointBuilder(configuration, 'rum')!.build('fetch', DEFAULT_PAYLOAD)).toContain(
        `o2-api-key=${replica.clientToken}`
      )
    })
  ...
  describe('buildEndpointUrl', () => {
    it('builds proxy URL from a string proxy', () => {
      ...
      ).toBe('https://proxy.io/path?o2forward=%2Fapi%2Fv2%2Frum%3Ffoo%3Dbar')
    })
    ...
    it('normalizes a relative proxy URL', () => {
      expect(buildEndpointUrl({ proxy: '/path', site: undefined, path: '/api/v2/rum', parameters: 'foo=bar' })).toBe(
        `${location.origin}/path?o2forward=%2Fapi%2Fv2%2Frum%3Ffoo%3Dbar`
      )
    })
    ...
    it('adds the subdomain forwarding hint when provided', () => {
      expect(
        ...
        'https://proxy.io/path?o2forward=%2Fapi%2Fv2%2Fprofiling%2Fquota%3Fsession_id%3Dabc&o2forwardSubdomain=quota'
      )
    })

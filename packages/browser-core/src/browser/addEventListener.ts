...
export type TrustableEvent<E extends Event = Event> = E & { __o2IsTrusted?: boolean }
...
export function addEventListeners<Target extends EventTarget, EventName extends keyof EventMapFor<Target> & string>(
  ...
  const listenerWithMonitor = monitor((event: TrustableEvent) => {
    if (!event.isTrusted && !event.__o2IsTrusted && allowUntrustedEventsFromConfiguration === false) {
      return
    }
    if (once) {
      stop()

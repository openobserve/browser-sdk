...
Copy `packages/browser-rum-slim/bundle/openobserve-rum-slim.js` into your Salesforce project as `staticresources/datadog_rum_slim.js`.
...
import openobserveRumSlim from '@salesforce/resourceUrl/datadog_rum_slim'
...
export default class DatadogInit extends NavigationMixin(LightningElement) {
  ...
  loadDatadogRum() {
    return loadScript(this, openobserveRumSlim).then(() => {
      const initConfig = {

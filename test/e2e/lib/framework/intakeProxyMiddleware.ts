...
function computeIntakeRequestInfos(req: express.Request): IntakeRequestInfos {
  const o2forward = req.query.o2forward as string | undefined
  if (!o2forward) {
    throw new Error('o2forward is missing')
  }
...
function forwardIntakeRequestToDatadog(req: express.Request): Promise<any> {
  return new Promise<void>((resolve) => {
    const o2forward = req.query.o2forward! as string
    if (!/^\/api\/v2\//.test(o2forward)) {
      throw new Error(`Unsupported o2forward: ${o2forward}`)
    }
    const options = {
      method: 'POST',
      headers: {
    ...
    const datadogIntakeRequest = https.request(new URL(o2forward, 'https://browser-intake-datadoghq.com'), options)

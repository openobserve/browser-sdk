...
test.describe('vital collection', () => {
  createTest('send custom duration vital')
    ...
    .run(async ({ flushEvents, intakeRegistry, page }) => {
      await page.evaluate(() => {
        window.O2_RUM!.startDurationVital('foo')
        return new Promise<void>((resolve) => {
          setTimeout(() => {
            window.O2_RUM!.stopDurationVital('foo')
            resolve()
          }, 5)
        })
      })
  ...
  createTest('send two simultaneous duration vitals using vitalKey')
    ...
    .run(async ({ flushEvents, intakeRegistry, page }) => {
      await page.evaluate(() => {
        const key1 = 'key-1'
        const key2 = 'key-2'
        window.O2_RUM!.startDurationVital('foo', { vitalKey: key1 })
        window.O2_RUM!.startDurationVital('foo', { vitalKey: key2 })
        return new Promise<void>((resolve) => {
          setTimeout(() => {
            window.O2_RUM!.stopDurationVital('foo', { vitalKey: key1 })
            window.O2_RUM!.stopDurationVital('foo', { vitalKey: key2 })
            resolve()
          }, 5)
        })
      })
  ...
  createTest('send operation step vital')
    ...
    .run(async ({ flushEvents, intakeRegistry, page }) => {
      await page.evaluate(() => {
        window.O2_RUM!.startOperation('foo')
      })
      await flushEvents()

...
test.describe('multi-tab sessions', () => {
  createTest('session expiry and renewal across tabs')
    ...
    .run(async ({ page, browserContext, baseUrl }) => {
      ...
      const tab1Session = await page.evaluate(() => window.O2_RUM!.getInternalContext()?.session_id)
      expect(tab1Session).toBeDefined()

      const page2 = await browserContext.newPage()
      await page2.goto(baseUrl)

      const tab2Session = await page2.evaluate(() => window.O2_RUM!.getInternalContext()?.session_id)
      ...
      const tab1AfterExpiry = await page.evaluate(() => window.O2_RUM!.getInternalContext()?.session_id)
      const tab2AfterExpiry = await page2.evaluate(() => window.O2_RUM!.getInternalContext()?.session_id)
      ...
      const tab1Renewed = await page.evaluate(() => window.O2_RUM!.getInternalContext()?.session_id)
      ...
      const tab2AfterRenewal = await page2.evaluate(() => window.O2_RUM!.getInternalContext()?.session_id)
      ...
      const tab2Renewed = await page2.evaluate(() => window.O2_RUM!.getInternalContext()?.session_id)

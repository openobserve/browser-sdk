  ...
  test.describe('sync loading', () => {
    createTest('should be fetched and applied')
      ...
      .run(async ({ page }) => {
        ...
        const initConfiguration = await page.evaluate(() => window.O2_RUM!.getInitConfiguration()!)
    ...
    createTest('should resolve user context')
      ...
      .run(async ({ page }) => {
        await waitForRemoteConfigurationToBeAppliedSync(page)
        const user = await page.evaluate(() => window.O2_RUM!.getUser())
    ...
    createTest('should resolve global context')
      ...
      .run(async ({ page }) => {
        await waitForRemoteConfigurationToBeAppliedSync(page)
        const globalContext = await page.evaluate(() => window.O2_RUM!.getGlobalContext())

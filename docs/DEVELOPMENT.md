...
node scripts/json-schemas.ts --update YOUR_RUM_EVENTS_FORMAT_BRANCH --build
```
...
Script exited with error: Error: TypeScript 3.8.2 compatibility compatibility broken
    ...
  node_modules/@openobserve/browser-rum-core/cjs/index.d.ts(15,46): error TS1005: ',' expected.
```

Reproduce locally with `yarn test:compat:tsc`.

Check the file mentioned in the error (e.g. `node_modules/@openobserve/browser-rum-core/cjs/index.d.ts`). The likely cause is using TypeScript syntax too recent for 3.8.2. For example, combining type and non-type exports on a single line:

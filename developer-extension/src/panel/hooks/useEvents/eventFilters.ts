...
function filterOutdatedVersions(events: SdkEvent[]): SdkEvent[] {
  ...
  for (const event of events) {
    if (isRumViewEvent(event)) {
      ...
      } else if (otherEvent._o2.document_version < event._o2.document_version) {
        upToDateEvents.set(event.view.id, event)
...
function deduplicateViewAndViewUpdate(events: SdkEvent[]): SdkEvent[] {
  ...
  for (const event of events) {
    if (isRumViewUpdateEvent(event) && event.view?.id && event._o2?.document_version) {
      viewUpdateVersions.add(`${event.view.id}:${event._o2.document_version}`)
    }
  }
  ...
  return events.filter((event) => {
    if (isRumViewEvent(event) && viewUpdateVersions.has(`${event.view.id}:${event._o2.document_version}`)) {
      return false
    }
    return true
  })
